#ifndef MANAGER_H
#define	MANAGER_H

#include <glib-object.h>
#include <glib.h>
#include <gio/gio.h>
#include "adapter.h"

G_BEGIN_DECLS

G_DECLARE_FINAL_TYPE(BztManager, bzt_manager, BZT, MANAGER, GDBusObjectManagerClient)
struct dummy;

#define BZT_TYPE_MANAGER bzt_manager_get_type()

#define BZT_MANAGER_DBUS_PATH       "/"
#define BZT_MANAGER_DBUS_INTERFACE  "org.freedesktop.DBus.ObjectManager"

BztManager *bzt_manager_new(GCancellable *cancel, GError **error);
    
BztAdapter *bzt_manager_find_adapter(BztManager *self, const gchar *pattern, GError **error);

GList *bzt_manager_get_adapters(BztManager *self);
GList *bzt_manager_get_devices(BztManager *self, const gchar *adapter_pattern);

G_END_DECLS
#endif	/* MANAGER_H */

